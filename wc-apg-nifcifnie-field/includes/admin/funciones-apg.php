...
function apg_nif_estilo( $hook ) {
	if ( isset( $_SERVER['REQUEST_URI'] ) ) {
		...
		if ( false !== strpos( $request_uri, 'wc-apg-nifcifnie-field' ) || false !== strpos( $request_uri, 'plugins.php' ) ) {
			...
			wp_register_style( 'apg_nif_hoja_de_estilo', plugins_url( 'assets/css/style.css', DIRECCION_apg_nif ), array(), VERSION_apg_nif, 'all' );
	...
	if ( 'woocommerce_page_wc-admin' === $hook ) {
		wp_enqueue_script( 'wc-apg-clientes-nif', plugins_url( 'assets/js/clientes-nif.js', DIRECCION_apg_nif ), array( 'wp-hooks', 'wp-api-fetch', 'wc-admin-app' ), VERSION_apg_nif, true );
...
add_filter(
	...
	function ( $response, $report, $request ) {
